export default {
    myFun1: async (currentRow) => {
        ...
        if (printerInfo.length > 0) {
            ...
            await storeValue(
                'printerItemsList',
                JSON.parse(JSON.stringify(itemsList).replaceAll("'", "''"))
            );
        ...
        const orderGroupIDs = [
            await get_printer_line_item.data[0].OrderGroupID,
        ];
        const groupType = orderGroupIDs[0].slice(
            0,
            orderGroupIDs[0].indexOf('_')
        );

        const vendorID = await (async () => {
            if (
                currentRow.AssignTo === 'PAUSED' ||
                currentRow.AssignTo === 'CANCELLED'
            ) {
                return currentRow.AssignTo;
            }
            return (
                await getVendorID.run({
    ...
    updateAllItemsVendor: async () => {
        ...
        if (printerInfo.length > 0) {
            const allItems = await get_selected_printer_item_all.run({
                invoiceID: Table2.selectedRow.InvoiceNo,
            });
            const itemsList = allItems.map((item) => {
                return {
                    printerId: printerInfo[0].Id,
    ...
    clearOrderGroupVendor: async (currentRow) => {
        ...
        const orderGroupIDs = (await get_printer_line_item.run()).map(
    ...
    updateOrderGroupVendors: async () => {
        ...
        for (const currentRow of Table3Copy.tableData) {
            if (currentRow.SubItemID === null && currentRow.itemid !== null) {
                await storeValue('rowUpdate', currentRow);
                orderGroupIDs.push(
                    (await get_printer_line_item.run())[0].OrderGroupID
                );
            }
        }
        ...
        for (const orderGroupID of orderGroupIDs) {
            if (orderGroupID !== 'null') {
                const groupType = orderGroupID.slice(
                    0,
                    orderGroupID.indexOf('_')
                );
                if (groupType === 'letter') {
        ...
        const vendorID = await (async () => {
            if (
                appsmith.store.MBitem === 'PAUSED' ||
                appsmith.store.MBitem === 'CANCELLED'
            ) {
	...
	classStr(group) {
        if (group.destinationCountryCode === 'CA') {
            if (
                group.mailingClass === 'standard_class' ||
                group.mailingClass === 'ca_post_personalized'
            ) {
                return 'Personalized';
        ...
        if (group.destinationCountryCode === 'GB') {
            if (
                group.mailingClass === 'standard_class' ||
                group.mailingClass === 'royal_mail_second_class'
            ) {
                return 'Second Class';
        ...
        if (group.destinationCountryCode === 'AU') {
            if (
                group.mailingClass === 'standard_class' ||
                group.mailingClass === 'au_post_second_class'
            ) {
                return 'Second Class';
            }
        ...
        if (
            group.mailingClass === 'standard_class' ||
            group.mailingClass === 'usps_standard_class'
        ) {
        ...
        if (
            ...
            ((group.destinationCountryCode !== 'US' &&
                ...
                (group.destinationCountryCode !== orgCountryCode &&
                    group.destinationCountryCode !== 'AS' &&
                    orgCountryCode === 'US'))
        ) {
            /** @type {LineItemWithoutID} */
            const intlItem = {
    ...
    letterGroupsLineItems(orgNames) {
        ...
        const groupLineItems = (group) => {
            ...
            const orgCountryCode =
                orgs.data.find((org) => org._id === group.organization)
                    .countryCode ?? 'US';
            ...
            const innerItems = this.baseGroupItems(
                group,
                orgName,
                orgCountryCode
            );
            ...
            if (sheetCount > 1) {
                ...
                if (sheetCount > 6) {
                    const sheetRange =
                        sheetCount <= 15
                            ? '7 - 15'
                            : sheetCount > 15 && sheetCount <= 60
                            ? '16 - 60'
                            : sheetCount > 60 && sheetCount <= 100
                            ? '61 - 100'
                            : sheetCount > 100 && sheetCount <= 150
                            ? '101 - 150'
                            : sheetCount > 150 && sheetCount <= 300
                            ? '151 - 300'
                            : sheetCount > 300 && sheetCount <= 500
                            ? '301 - 500'
                            : sheetCount > 500 && sheetCount <= 1000
                            ? '501 - 1000'
                            : sheetCount > 1000 && sheetCount <= 1500
                            ? '1001 - 1500'
                            : sheetCount > 1500 && sheetCount <= 2000
                            ? '1501 - 2000'
                            : 'over 2000';
                    /** @type {LineItem} */
                    ...
                    if (
                        (group.organization === 'org_3pWY3piAx4H7XNrpY25RU2' &&
                            sheetCount >= 20) ||
                        (group.organization !== 'org_3pWY3piAx4H7XNrpY25RU2' &&
                            sheetCount > 6 &&
                            group.organization !== 'org_wUB4oa68YQgdQNYTeiSYNQ')
                    ) {
            ...
            if (group.returnEnvelope) {
                let envelopeName = '';
                if (
                    group.returnEnvelope ===
                    'return_envelope_9KgsgkKFe5YYuT2QLuZbdq'
                ) {
                    envelopeName = 'SylvanHealth Ret Envelope';
                } else if (
                    group.returnEnvelope ===
                    'return_envelope_mkG3NHTZZUnaz6jahkJRX5'
                ) {
            ...
            if (customEnvelopeOrgs.includes(orgName)) {
                let envelopeName = '';
                switch (orgName) {
                    ...
                    case 'Highland Health Direct':
                        envelopeName =
                            'Highland Health Direct Full Front Window Envelope';
                        break;
                    ...
                    case 'ERC Partners, LLC':
                        envelopeName =
                            'ERC Partners, LLC (Garett Law) Custom Envelope';
                        break;
                    case 'AnsibleHealth':
    ...
    formatPostcardCollateral: (item, group, classStr) => {
        ...
        if (group.organization === 'org_iWccrJCPmbqmVbz1j1yNXg') {
            if (
                item.productDesc.includes('6x11') ||
                item.productDesc.includes('11x6_reduced')
            ) {
                return {
                    ...item,
                    mailType: 'Custom',
                ...
                item.productDesc.includes('6x9') ||
                item.productDesc.includes('9x6_reduced')
            ) {
                return {
                    ...item,
                    mailType: 'Custom',
    ...
    selfmailerGroupsLineItems(orgNames) {
        const groupLineItems = (group) => {
            ...
            const sizeStr =
                group.size == '8.5x11_bifold' ? '8.5x11 - Bifold' : group.size;
    ...
    updateOrderInfo: async () => {
            ...
            if (order.groupID === null) {
                ...
                if (updateGroup === true) {
                    await storeValue(
                        'newList',
                        JSON.parse(
                            JSON.stringify(groupToAdd).replaceAll("'", "''")
                        )
                    );
                    await Promise.all([
                    ...
                    showAlert(
                        `${groupToAdd[0].groupID} added for ${order.orgID}.`
                    );
        ...
        if (updateGroup === true) {
            ...
            await storeValue(
                'newList',
                JSON.parse(JSON.stringify(groupToAdd).replaceAll("'", "''"))
            );
            await Promise.all([
                set_customerprice.run(),
                set_customerlineitems.run(),
        ...
        await storeValue(
            'newList',
            JSON.parse(JSON.stringify(newCollateral).replaceAll("'", "''"))
        );
        ...
        Promise.all([
            ...
            showAlert(
                'Well done! Your action has been processed successfully.',
                'success'
            );
        });
    },
